...
def decompose_scene(req: TextRequest):
    if not client:
        return {"error": "Нет API ключа на сервере"}
    try:
        prompt = f"""Разбей сцену на слои. Верни СТРОГИЙ JSON без markdown-разметки.
Формат:
{{
  "layers": [
    {{"name": "Background", "role": "Фон", "kind": "background", "prompt": "детальный промпт для фона"}},
    {{"name": "Subject", "role": "Объект", "kind": "object", "prompt": "детальный промпт для объекта"}}
  ]
}}
Сцена: {req.prompt}"""
        ...
        # Очищаем ответ от маркдауна, если модель его всё же добавит
...
def generate_layer(req: LayerRequest):
    ...
    try:
        ...
        return {"error": "Модель не вернула картинку. Попробуй упростить промпт."}
    except Exception as e:
        return {"error": str(e)}
